...
import { News, NTargetOrgGroup } from '@/controllers'
...
let nTargetOrgGroupInstance = new NTargetOrgGroup()
 ...
 * @return {Object}
 */
router.post('/news_target/org_group', async function (req: any, res: express.Response) {
    ...
    try {
         ...
         * insert目标群体
         */
        await nTargetOrgGroupInstance.remove({ news_id: news_id }, t)
        const insertRes = await nTargetOrgGroupInstance.insert(insertData, t)
        await t.commit()
 ...
 * @return {Object}
 */
router.get('/news_target/org_group/:news_id', async function (req: any, res: express.Response) {
    const { user_id } = <sessionInterface>req['payload']
    ...
    const findAllRes = await nTargetOrgGroupInstance.getAll({ news_id: news_id })
    // 数据格式化
